// The honest declaration: `@idfkit/idfkit/language` is `@idfkit/language`, whole.
//
// @idfkit/language is an OPTIONAL peer dependency. Installing `@idfkit/idfkit` does not
// install it, so this re-export is unresolvable until it is added:
//
//     npm install @idfkit/language
//
// TypeScript only reads this file when something imports `@idfkit/idfkit/language`, so a
